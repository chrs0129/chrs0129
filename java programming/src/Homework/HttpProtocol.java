...
public class HttpProtocol {
    public static void main(String[] args) {
        int statusCode = 202;
        String output = (statusCode == 200)? "Ok" :(statusCode == 201)? "Created" :(statusCode == 202)? "Acsepted"
                :(statusCode == 301)? "Moved Permanently" :(statusCode == 303)? "See Order" :(statusCode == 304)? "Not Modified"
                :(statusCode == 307)? "Temporary Redirect" :(statusCode == 400)? "Bad Request" :(statusCode == 401)? "Unauthorized"
                :(statusCode == 403)? "Unauthorized" :(statusCode == 404)? "Not Found" :(statusCode == 410)? "Gone"
                :(statusCode == 500)? "Internal Server Error" :(statusCode == 503)? "Service Unavailable" : "Invalid input";
